...
# # 设置 GT 文件夹和结果图像文件夹
# GT_FOLDER = "/data/myy/IR/GOPRO/test/gt"  # 真实图像文件夹
# RESULT_FOLDER = "/data/myy/ACL0124/results/goprotestcrop/test"  # 生成的图像文件夹
...
# # 遍历图像进行 PSNR 和 SSIM 计算
# for gt_name, result_name in zip(gt_images, result_images):
#     gt_path = os.path.join(GT_FOLDER, gt_name)
#     result_path = os.path.join(RESULT_FOLDER, result_name)
    
#     # 读取并转换为 tensor
#     gt = ToTensor()(Image.open(gt_path).convert("RGB"))
#     result = ToTensor()(Image.open(result_path).convert("RGB"))
    
#     # 计算 PSNR 和 SSIM  IF Rain Y=True else False
#     psnr_value = calculate_psnr(result * 255, gt * 255, crop_border=0, test_y_channel=True)
#     ssim_value = calculate_ssim(result * 255, gt * 255, crop_border=0, test_y_channel=True)
    
#     psnrs.append(psnr_value)
#     ssims.append(ssim_value)
    
#     print(f"{result_name}: PSNR={psnr_value:.4f}, SSIM={ssim_value:.4f}")
